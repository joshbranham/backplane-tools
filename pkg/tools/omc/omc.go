...
const (
	toolChecksumAssetName = "checksums.txt"
)

// Tool implements the interface to manage the 'omc' binary
type Tool struct {
	base.Github
}
...
func New() *Tool {
	t := &Tool{
		Github: base.Github{
			Default: base.NewDefault("omc"),
			Source:  github.NewSource("gmeghnag", "omc"),
		},
	}
	return t
}
...
func (t *Tool) Install() error {
	...
	matches = github.FindAssetsContaining([]string{".tar.gz"}, matches)
	if len(matches) != 1 {
	...
	binarySum, err := utils.Md5sum(toolArchiveFilepath)
	if err != nil {
